...
alpha     <- 0.55
bar_theta <- 1000
k         <- #function(x){exp(-x)}#function(x){(1-1/x^2)}
...
for (j in 1:m){
  ...
  for (i in 1:n) {
    ...
    if (theta_l < bar_theta) { # r=(1,0)
      if (theta_C > theta_I * (1 - k(i))) {
        theta_l <- max(theta_min, theta_I * (1 - k(i)))
      } else {
        theta_l <- max(theta_l, theta_C / (1 - k(i)))
      }
...
esp <- hist(his, 
            ...
            ylab = "Frecuencia",
            main = substitute(kappa == cuerpo, list(cuerpo = body(k))))
